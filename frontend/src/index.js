...
class App extends React.Component {
  ...
  componentWillMount() {
    get_active().then(a => {
      this.setState({ ...this.state, active: a });
    });
    get_thresholds().then(t => {
      this.setState({ ...this.state, thresholds: t });
    });
  }
    ...
    return (
      <div className="popup">
        <div className="popup-switch">
          Active:{" "}
          <Switch
            key={this.state.active.toString}
        ...
        <div className="popup-slide">
          Threshold:{" "}
          <Slider
            key={this.state.thresholds.toString}
            range
            max={1}
            min={0}
            value={this.state.thresholds}
            onAfterChange={save_thresholds}
            onChange={t => this.setState({ ...this.state, thresholds: t })}
            step={0.01}
          />
        </div>
      </div>
